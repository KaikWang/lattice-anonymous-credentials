...
static void osig_handle_to_vec(poly_q_vec_d x_vec, uint32_t x) {
...
void osig_user_commit(poly_q_vec_d r[2], poly_q_vec_d cmt, const uint8_t msg[PARAM_M*PARAM_N/8], const user_pk_t *upk) {
  osig_user_commit_handle(r, cmt, msg, upk, 0);
}

/*************************************************
* Name:        osig_user_commit_handle
*
* Description: Compute hiding commitment to (usk | msg | x) by
*              cmt = (I|A')r + Ds.usk + D.msg + Dx.x
*
* Arguments:   - poly_q_vec_d *r: array of polynomial vectors for user commitment randomness (initialized)
*              - poly_q_vec_d cmt: polynomial vector for user commitment (initialized)
*              - const uint8_t *msg: pointer to input message byte array (allocated PARAM_M*PARAM_N/8 bytes)
*              - user_pk_t *upk: pointer to user public key structure
*              - uint32_t x: 32-bit revocation handle
**************************************************/
void osig_user_commit_handle(poly_q_vec_d r[2], poly_q_vec_d cmt, const uint8_t msg[PARAM_M*PARAM_N/8], const user_pk_t *upk, uint32_t x) {
  size_t i;
  uint8_t randomness_seed[SEED_BYTES];
  poly_q_mat_d_d A, Dx;
  poly_q_mat_d_m D;
  poly_q_vec_m m;
  poly_q_vec_d tmp, x_vec;
  ...
  // cmt = r[0] + A'.r[1] + Ds.usk + D.msg + Dx.x (but Ds.usk is already computed in upk)
  ...
  osig_handle_to_vec(x_vec, x);
  poly_q_mat_d_d_mul_vec_d(tmp, Dx, x_vec);
  poly_q_vec_d_add(cmt, cmt, tmp);
...
int osig_user_verify(const sep_sig_t *sig, const sep_pk_t *pk, const user_pk_t *upk, const uint8_t msg[PARAM_M*PARAM_N/8]) {
  return osig_user_verify_handle(sig, pk, upk, msg, 0);
}

/*************************************************
* Name:        osig_user_verify_handle
*
* Description: User verification of the obtained signature bound to a 32-bit revocation handle
*
* Arguments:   - const sep_sig_t *sig: pointer to the input signature structure
*              - const uint8_t *msg: pointer to input message byte array (allocated PARAM_M*PARAM_N/8 bytes)
*              - const sep_pk_t *pk: pointer to public key structure
...
* Returns 1 if signature could be verified correctly and 0 otherwise
**************************************************/
int osig_user_verify_handle(const sep_sig_t *sig, const sep_pk_t *pk, const user_pk_t *upk, const uint8_t msg[PARAM_M*PARAM_N/8], uint32_t x) {
  size_t i;
  int is_valid;
  ...
  // computing binding commitment bind_cmt = Ds.usk + D.msg + Dx.x (randomness already included in sig)
  poly_q_mat_d_m_mul_vec_m(bind_cmt, D, m);
  poly_q_vec_d_add(bind_cmt, bind_cmt, upk->t);
  osig_handle_to_vec(x_vec, x);
  poly_q_mat_d_d_mul_vec_d(tmp, Dx, x_vec);
  poly_q_vec_d_add(bind_cmt, bind_cmt, tmp);
